...
- (void)waterScroll:(UIScrollView *)scrollView
{
    ...
    if (_visibleRange.length > 0) {
        ...
        CGFloat topOutHeight = (topOutPoint).y+[[_resHeightDic objectForKey:
                                                 [NSNumber numberWithLong:_visibleRange.location]] floatValue];
    ...
    if (_visibleRange.location+_visibleRange.length <= _bottomDrawPointVector.size()-1) {
        if(_bottomDrawPointVector.size() > 0){
            ...
            if (bottomInHeight<BottomLineHeight) {
                UIView *view = [self drawView:bottomInPoint
                                        index:(_visibleRange.location+_visibleRange.length )];
...
- (void)loadMore
{
    if ([self.waterDelegate respondsToSelector:@selector(needLoadMoreByWaterView:)]) {
        ...
        if([self respondsToSelector:@selector(needLoadMoreByWaterView:)]){
            [self performSelector:@selector(needLoadMoreByWaterView:) withObject:self];
        }
    }
}
 ...
 *  @param resArr 视图数组
 */
- (UIView *)drawView:(CGPoint)drawPoint index:(long)index
{
    CGRect flowViewRect;
    flowViewRect.origin = drawPoint;
    flowViewRect.size = CGSizeMake(_resWidth, [[_resHeightDic objectForKey:[NSNumber numberWithLong:index]] floatValue]);
...
- (void)initialize
{
    ...
    //初始化视图
//    if(_topEntityVector )
    if(_topEntityVector.size() > 0 &&[_topEntityVector[0] isKindOfClass:[TopEntity class]]){
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if([_waterDelegate respondsToSelector:@selector(scrollViewDidScroll:)]){
        [_waterDelegate performSelector:@selector(scrollViewDidScroll:) withObject:scrollView];
    }
}

- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated{
    
//    if(_topEntityVector.size() > 0){
//        for(int i = 0;i < _topEntityVector.size();i++){
//            TopEntity *entity = _topEntityVector[i];
//            if(entity.index == index){
//                CGPoint point = CGPointMake(0, entity.point.y);
...
//    }
    if(_topEntityVector.size() > 0){
        ...
        CGPoint point = _bottomDrawPointVector[index];
        point.x = 0;
    
//        if(self.contentOffset.y > 0)
//        if(self.contentOffset.y < self.waterInfo.topMargin &&(index == 0|| index == 1))
//            point.y += self.waterInfo.topMargin;
        ...
        for(int i = 0; i < _topEntityVector.size() ; ++i){
            TopEntity *entity = _topEntityVector[i];
            if (entity.index == _topEntityVector.size() - 1) {
        ...
        if(self.contentSize.height > self.bounds.size.height){
            if(point.y  > self.contentSize.height - self.bounds.size.height){
                point.y = self.contentSize.height - self.bounds.size.height;
            }
        }
//        if(point.y + )
        [self setContentOffset:point animated:animated];
